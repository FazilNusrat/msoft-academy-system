    ...
    <template v-slot:header="props">
        ...
        <template v-slot:body-cell-actions="props" :dense="dense">
          <q-td key="actions" :props="props">
            ...
            <q-btn  size="sm" color="secondary" class="glossy  my_border_white_less q-ml-xs" v-show="noInfo?false:true" :icon="infoIcon?infoIcon:'info'" @click="info(props.row.id)" />
            <q-btn  size="sm" color="negative" class="glossy  my_border_white_less q-ml-xs" v-show="noDelete?false:true" icon="delete" @click="del(props.row.id)"  />
      ...
      <template v-slot:top-right>
        <q-input standout="bg-cyan-7" class="q-ml-sm" :loading="loading" dense debounce="300" @input="$emit('update:filter', $event)"  :value="filter" placeholder="Search">
      ...
      <template v-slot:top-left>
        ...
        <q-toggle label="M-Sizer" color="cyan-7" v-model="dense" icon="apps" />
      </template>
