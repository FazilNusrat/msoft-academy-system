<template>
  <div class="q-pa-md q-gutter-sm">
    ...
    <h-title>Staff Attendance Entry</h-title>
    ...
    <div>
      <n-table :title="$t('Staff Attendance List')" :loading="loading" :data="data" :pagination.sync="pagination" @del="del" @info="info" @edit="edit" :filter.sync="filter" :columns="columns" @request="onRequest" />
...
import NAddModal from 'src/components/modals/staffattendance/Add.vue'
import NEditModal from 'src/components/modals/staffattendance/Edit.vue'
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { name: 'name', align: 'center', label: 'Class', field: row=>row.name, sortable: true },
        { name: 'name', align: 'center', label: 'Section', field: row=>row.name, sortable: true },
        // { name: 'description',classes: 'bg-grey-2 ellipsis', align: 'center', label: 'Description', field: row=>row.description, sortable: true },
        { name: 'actions', label: 'Search By Keyword', classes: 'my_width10', sortable: false, align: 'center my_width20'},
